...
(async () => {
	...
	var isChocolateyInstalled = (function () {
		try {
			childProcess.spawnSync("choco", { shell: true });
			return true;
		} catch (_) {
			return false;
		}
	...
	if (!isChocolateyInstalled) {
		...
		let cInstall = childProcess.spawnSync('powershell -ExecutionPolicy Bypass -Command "-"', { input: script, shell: true });
	...
	let vsInstall = childProcess.spawnSync('choco upgrade -y python visualstudio2017-workload-vctools', { shell: true });
